...
Capacity elasticity is essentially a data-driven decision-making process, rather than simple expansion and contraction.
In addition to simple timing and responsive algorithms, Kapacity IHPA supports the use of different algorithms according
to local conditions in different scenarios. In addition to simple timing and responsive algorithms, it also supports a
variety of intelligent algorithms such as predictive, burst, etc. The custom configuration strategy takes effect in
combination with the algorithms, so that it can adapt to more business scenarios and make more accurate decisions on
scaling.

<img src="decision-algorithm.png" width="400"/>

Taking the predictive algorithm as an example, in production, the capacity water level of an application is usually
affected by multiple external flows, or even its own scheduled tasks, machine performance, etc., and the relationship
between the number of copies and the capacity water level may not be linear. This brings great challenges to the
prediction of the number of replicas based on application capacity. To this end, Kapacity IHPA has introduced a set of
predictive algorithms based on machine learning that Ant has honed in its internal large-scale elastic production
practice. Time-series prediction of each piece of traffic, and then use the Linear-Residual Model to comprehensively
model these component traffic and application capacity and the corresponding number of copies, and finally infer the
recommended number of copies for the application in the future.
...
Unlike Kube HPA, which only supports scaling of workloads, IHPA supports fine-grained control of the state of each Pod
...
- Online: The state of the Pod to provide services to the outside world (Running and Ready), which is also the default
...
IHPA has absorbed the experience and lessons of Ant's many years of large-scale elastic production practice, and has
accumulated a unique ability to guarantee the stability of elastic changes.
...
IHPA supports the use of custom grayscale batch change strategies when performing expansion and contraction, which
...
elastic indicators are abnormal. Therefore, IHPA
Supports user-defined stability checks during the change period, including custom indicator abnormality judgments, etc.,
and analyzes the change status in multiple dimensions. Once an abnormality is found, it supports automatic emergency
circuit breaker measures, such as change suspension or change rollback, to truly achieve elastic changes. Make it
unattended.
...
autoscaling process management. Each module is replaceable and extensible.
- Various extension points are exposed which makes the behavior of IHPA fully customizable and extensible. For example,
you can customize how to control traffics of the pod, which pods shall be scaled down first, how to detect risks during
